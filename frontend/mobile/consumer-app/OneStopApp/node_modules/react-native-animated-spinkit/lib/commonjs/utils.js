...
exports.createKeyframeEasingFunction = createKeyframeEasingFunction;
exports.anim = anim;
...
function createKeyframeEasingFunction(keyframes, easing) {
  return t => {
    for (let keyframeIndex = 1; keyframeIndex < keyframes.length; keyframeIndex++) {
...
function anim({
  ...
  value,
  keyframes = [0, 100],
  easing = _reactNative.Easing.bezier(0.42, 0.0, 0.58, 1.0),
  toValue = 100,
  infinite = true,
  delay = 0
}) {
  const timing = _reactNative.Animated.timing(value, {
    duration: duration,
  ...
  return infinite ? _reactNative.Animated.loop(timing) : timing;
}
...
function stagger(time, amount, animationConfig) {
  ...
  const animation = infinite ? _reactNative.Animated.loop(timing) : timing; // React Native only does 60fps
  ...
  for (let index = amount - 1; index >= 0; index--) {
    ...
    const value = index === 0 ? animationConfig.value : animationConfig.value.interpolate({
      inputRange,
      outputRange
    });
    values.push(value);
